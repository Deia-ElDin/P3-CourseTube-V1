...
class Instructor {
  ...
  generateId() {
    ...
    // Else => charAt(0).toLowerCase() + return, Ex. SuperSimpleDev
      ...
      channelId =
        fullChannelName.charAt(0).toLowerCase() + fullChannelName.substring(1);
      return channelId;
    }
  ...
  getCourseImgUrl(courseName) {
    const imgSentence = `images/instructors/${
      this.channelName
...
function coursesArray(objectName) {
  ...
  const londonAppBreweryArray = [
    [
      'London App Brewery',
      'https://www.youtube.com/channel/UCVD5Vh9LhLBxp3o1vRNyf_w',
      false,
    ],
...
function courseTube(input, targetCourse) {
  ...
  function htmlCode(instructor, course) {
    ...
    verified
      ? channelNameDiv.append(AnchorTagChannelNameSection, verifiedImg)
  ...
  if (SpeechRecognition) {
    ...
    recognition.onresult = (e) => {
      ...
      setTimeout(() => {
        ...
        document.getElementById('search-btn-container').style.backgroundColor =
          '#bebebe80';
      }, 2000);
      setTimeout(() => {
        document.getElementById('search-btn-container').style.backgroundColor =
